Class TrigTableDeg {

    public static void main (String argrs[]){
        double c,s,t; //temp double for cosine, sine and tangent so they can be rounded 
        long d = 0l; // degrees
        double r =0, rround = 0;// radian measure
        double pi = 3.14159265358979323846;
        int i = 15; //increment r 15 degrees equals pi radians
        // time calcution  
        final long ms0 , ms1;
            final float sec0, sec1;
            
            ms0 = System.currentTimeMillis();
            sec0 = ms0 / 1000000000.0f;
            System.out.Println("radian measurement is represented as rad");
            System.out.Println("\trad \t|\tcos\|tsin\t|\ttan" );
            System.out.Println(
            "-----------------------------------------------------------------------" );
            // 9223372036854775807   18446744073709551614
            while (d <=  10000000){
                //convert angle t to radians r
                r = d * (pi/180.0);
                c = Math.cos(r);
                c = (Math.round(c *1000.0))/100.0; 
                s = Math.sin (r); 
                s = (Math.round(s *1000.0))/100.0; 
                t = Math.tan(r);
                t = (Math.round(t *1000.0))/100.0; 
                rround = (Math.round(r*1000.0))/100.0; 
                
                if (c == 0.0)
                    { System.out.println("\tt"+d+ "\t|\t"+ c +"\t|\t"+ s + "\t|\t UN ");
                    }eles{
                      System.out.println("\tt"+d+ "\t|\t"+ c +"\t|\t"+ s + "\t|\t  + t);
                    }
                d = d + i;
             }
              // time caluclation 
                  ms1 = System.currentTimeMillis();
                  sec1 = ms1 / 1000000000.0f
                  System.out.format("%.10f", sec0);
                  System.out.println();
              // end of time calculation
         }
    } 
             
